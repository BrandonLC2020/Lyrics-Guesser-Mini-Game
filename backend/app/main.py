...
# Relative imports from your other files
from .models import NewRoundResponse, GuessRequest, GuessResult
from .config import SECRET_KEY, SONG_DATABASE
...
@app.get("/api/game/new", response_model=NewRoundResponse)
async def start_new_round():
    ...
    game_token = serializer.dumps({"artist": artist, "title": title})
...
@app.post("/api/game/submit", response_model=GuessResult)
async def submit_guess(request: GuessRequest):
    # 1. Decrypt the token to get the real answer
    try:
        data = serializer.loads(request.game_token)
